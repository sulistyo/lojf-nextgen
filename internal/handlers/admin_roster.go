...
func AdminRoster(t *template.Template) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		now := time.Now()
		from := parseDate(f.From, now.AddDate(0, 0, -30))
		to := parseDate(f.To, now.AddDate(0, 0, 30))
		...
		if f.ClassID != "" {
			if cid, err := strconv.Atoi(f.ClassID); err == nil && cid > 0 {
		...
		if f.Status != "" {
			...
			case "confirmed", "waitlisted", "canceled":
				q = q.Where("registrations.status = ?", f.Status)
			case "checked-in":
		...
		// Sort: class (date, name) → created_at → status weight
		...
		// Compute waitlist rank per class (FIFO)
		ranks := map[uint]int{}
		for i := range rows {
